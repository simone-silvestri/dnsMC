...
void temp_fluid_copy(cudaTextureObject_t *tex_tempf_d, Var_CPU *varCPU)
{
	...
	static cudaTextureObject_t tex_temp[grid_num];

    for(int grd = 0; grd<grid_num; grd++)
    {
    	//First grid temperature
    	const cudaExtent extentT = make_cudaExtent(varCPU[grd].im+2, varCPU[grd].jm+2, varCPU[grd].km+2);
    	...
    	copyParamsT.srcPtr   = make_cudaPitchedPtr((void*)varCPU[grd].T, extentT.width*sizeof(myfloat), extentT.width, extentT.height);
        ...
        cudaCreateTextureObject(&tex_temp[grd], &texRes, &texDescr, NULL);
        cudaCheckErrors("Bind fail");
    }
    cudaMemcpy(tex_tempf_d , &tex_temp , grid_num*(sizeof(cudaTextureObject_t)) ,cudaMemcpyHostToDevice);
}
